 ...
 */
static method_file_t *resolve_clinit(class_file_t *class) {
    for(int i=0;i<class->methods_count;i++) {
        method_file_t *method = &class->methods[i];
...
static char *resolve_class_name(class_file_t *class, u2 class_index){ 
 ...
 * 查询自己类中的field，用于初始化
 */
static field_file_t *find_field(class_file_t *class, cp_info_t *cp_info) {
    ...
    for(u2 i = 0; i < class->fields_count; i++) { 
        field_file_t *field = &class->fields[i];
...
class_file_t *load_class(const char *class_file, jvm_thread_t *thread) {
    // load from cache
    class_file_t *class = NULL;
    for(size_t i=0;i<g_class_list->size;i++) {
        class_file_t *tmp = (class_file_t*)arraylist_get(g_class_list, i);
...
void link_class(jvm_thread_t *thread, class_file_t *class) { 
    ...
    if(strcmp("java/lang/Class", class->class_name) != 0) {
        // 创建一个java_mirror
        class_file_t *java_lang_class = load_class("java/lang/Class", thread);
...
void ensure_class_initialized(class_file_t *class, jvm_thread_t *thread) {
    ...
    if(class->super_class != 0) {
        ...
        class_file_t *super_class = load_class(super_class_name, thread);
        ensure_class_initialized(super_class, thread);
        class->super = super_class;
    }
    if(class->interface_count > 0) {
        printf("interface count: %d\n", class->interface_count);
        class->interface_class = calloc(class->interface_count, sizeof(class_file_t*));
        for(u2 i=0;i<class->interface_count;i++) {
            ...
            class_file_t *interface_class = load_class(interface_name, thread);
            class->interface_class[i] = interface_class;
        }
    }

    method_file_t *clinit = resolve_clinit(class);
    if(clinit) {
...
void bootstrap(project_t *project) {
    ...
    if(g_class_list->size > 0) {
        for(size_t i = 0; i < g_class_list->size; i++) {
            class_file_t *class = arraylist_get(g_class_list, i);
            int is_white = 0;
