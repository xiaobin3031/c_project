...
attribute_t *read_attributes_bytes(class_bytes_t *class_bytes, u2 attr_count, cp_info_t *cp_pools) {
    ...
    for(u2 i = 0; i < attr_count; i++) {
        ...
        if(attr.attribute_length > 0 
            ...
            attr.info = read_bytes_bytes(class_bytes, attr.attribute_length);
        }
        if(strcmp(attr_name, "Code") == 0) {
            attr.tag = ATTR_CODE;
            attr_code_t *code_attr = malloc(sizeof(attr_code_t));
            ...
            if(code_attr->exception_table_length > 0) {
                ...
                for(u2 j = 0; j < code_attr->exception_table_length; j++) {
                    ...
                    code_attr->exception_table[j].handler_pc = read_bytes_u2(class_bytes);
                    code_attr->exception_table[j].catch_type = read_bytes_u2(class_bytes);
                }
            } else {
                code_attr->exception_table = NULL;
            }
            code_attr->attributes_count = read_bytes_u2(class_bytes);
            code_attr->attributes = read_attributes_bytes(class_bytes, code_attr->attributes_count, cp_pools);
            attr.info = (u1 *)code_attr;
        } else if(strcmp(attr_name, "StackMapTable") == 0) {
            attr.tag = ATTR_STACK_MAP_TABLE;
        } else if(strcmp(attr_name, "Signature") == 0) {
