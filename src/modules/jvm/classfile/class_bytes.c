...
class_file_bytes_t *class_bytes_new(u1 *bytes, size_t len) {
    char *memory = malloc(sizeof(class_file_bytes_t) + sizeof(u1) * len);
    class_file_bytes_t *class_bytes = (class_file_bytes_t *)memory;
    class_bytes->bytes = (u1*)(memory + sizeof(class_file_bytes_t));
